...
import { isToday, isTomorrow, isAfter, addDays, parseISO, isSameDay, format, differenceInSeconds } from 'date-fns';
    ...
    <div className="flex gap-3 sm:gap-6 justify-center items-center mt-8">
      {[
        ...
        <div key={item.label} className="flex flex-col items-center">
          ...
          <span className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-[0.2em] mt-3 font-semibold">{item.label}</span>
        </div>
      ))}
    </div>
  );
...
export default function Home() {
  ...
  const getGroupedFixtures = () => {
    if (!filteredFixtures) return { today: [], upcoming: [], nextMatch: null, popularTeams: [] };

    const now = new Date();
    const today: typeof filteredFixtures = [];
    const upcoming: typeof filteredFixtures = [];
    ...
    filteredFixtures.forEach(fixture => {
      ...
      else if (isAfter(date, now)) upcoming.push(fixture);
    ...
    return { 
      ...
      upcoming, 
      nextMatch,
      popularTeams: Array.from(teamsMap.values()).slice(0, 12) // Top 12 unique teams
    };
  };

  const { today, upcoming, nextMatch, popularTeams } = getGroupedFixtures();
    ...
    >
      ...
      <section className="relative pt-12 pb-20 sm:pt-24 sm:pb-32 px-4 overflow-hidden border-b border-border/50 bg-background">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          ...
          >
            FIFA World Cup <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-cyan-400">2026</span>
          </motion.h1>
          <motion.p 
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
          ...
          >
            Follow every fixture, team, and match of the FIFA World Cup 2026 in real-time.
          </motion.p>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 mt-12 space-y-16">
          ...
          <div className="w-full sm:w-auto">
            <Popover>
              ...
              >
                ...
                {selectedDate ? format(selectedDate, "MMMM do, yyyy") : <span className="text-base">Filter by Date</span>}
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0 bg-card border-border shadow-2xl rounded-2xl overflow-hidden" align="end">
                <Calendar
                  mode="single"
        ...
        {isLoading ? (
          ...
          <div className="text-center p-8 bg-destructive/10 rounded-2xl text-destructive border border-destructive/20">
            ...
            <p className="text-muted-foreground">Please check your API token or connection.</p>
          </div>
        ) : (
          <>
            ...
            {(searchQuery || selectedDate) ? (
              ...
              <>
                ...
                <section>
                  ...
                  {today.length > 0 ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      {today.map((fixture) => <FixtureCard key={fixture.id} fixture={fixture} />)}
                    </div>
                  ) : (
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                      {popularTeams.map((team, idx) => (
                        ...
                        >
                          <Link href={`/team/${team.id}`} className="group block bg-card hover:bg-card/80 border border-border hover:border-primary/50 transition-all duration-300 rounded-2xl p-6 text-center shadow-lg hover:shadow-primary/10 hover:-translate-y-1">
                            <div className="w-16 h-16 mx-auto mb-4 bg-background rounded-full flex items-center justify-center p-3 border border-border group-hover:border-primary/50 transition-colors">
                              <img src={team.crest} alt={team.name} className="max-w-full max-h-full object-contain drop-shadow-md group-hover:scale-110 transition-transform" />
                            </div>
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {upcoming.slice(0, 10).map((fixture) => <FixtureCard key={fixture.id} fixture={fixture} />)}
                  </div>
                  {upcoming.length > 10 && (
                    <div className="text-center mt-8">
                      <Button variant="outline" className="bg-card border-border hover:bg-card hover:border-primary rounded-full px-8 py-6 text-base font-medium">
                        View All Fixtures
                      </Button>
                    </div>
                  )}
